<template>
  <div class="detail">
    ...
    <div class="detail-content">
          ...
          <div class="thing-infos-view">
            <div class="thing-infos">
              ...
              <div class="thing-info-box">
                <div class="thing-state">
                  <span class="state hidden-sm">{{ lang.goodsStatus[this.$store.state.language] }}</span>
                  <span>{{ lang.sale[this.$store.state.language] }}</span>
                </div>
                ...
                <div class="translators flex-view" style="">
                  <span>{{ lang.category[this.$store.state.language] }}</span>
                  <span class="name">{{ detailData.classification_title }}</span>
                </div>
                <div class="translators flex-view" style="">
                  <span>{{ lang.store[this.$store.state.language] }}</span>
                  <span class="name">{{ detailData.repertory }}</span>
                </div>
                <a-space>
                  <button class="buy-btn" @click="handleOrder(detailData)">
                    <img src="@/assets/images/add.svg" />
                  <span>{{ lang.buy[this.$store.state.language] }}</span>
                </button>
                  <button class="buy-btn" @click="handleCart(detailData)">
                    <img src="@/assets/images/add.svg" />
                  <span>{{ lang.addcart[this.$store.state.language] }}</span>
                </button>
                </a-space>
            ...
            <div class="thing-counts hidden-sm">
              <div class="count-item flex-view pointer" @click="addToWish()">
                ...
                <div class="count-box flex-view">
                  <div class="count-text-box">
                    <span class="count-title">{{ lang.wish[this.$store.state.language] }}</span>
                  </div>
              ...
              <div class="count-item flex-view pointer" @click="collect()">
                ...
                <div class="count-box flex-view">
                  <div class="count-text-box">
                    <span class="count-title">{{ lang.collect[this.$store.state.language] }}</span>
                  </div>
                  <div class="count-num-box">
              ...
              <div class="count-item flex-view" @click="share()">
                ...
                <div class="count-box flex-view">
                  <div class="count-text-box">
                    <span class="count-title">{{ lang.share[this.$store.state.language] }}</span>
                  </div>
                  <div class="count-num-box">
      ...
      <div v-if="this.$store.state.language === 'zh'" class="detail-content-bottom">
      ...
      <div v-if="this.$store.state.language === 'en'" class="detail-content-bottom">
        <div class="thing-content-view flex-view">
          <div class="main-content">
            <div  class="order-view main-tab">
          <span class="tab"
                :class="selectTabIndex===index? 'tab-select':''"
                v-for="(item,index) in enTabData"
                :key="index"
                @click="selectTab(index)">
            {{ item }}
          </span>
            ...
            <!--评论-->
            <div class="thing-comment" :class="selectTabIndex > 0? '':'hide'">
              <div class="title">Send new comment</div>
              <div class="publish flex-view">
                <img src="@/assets/images/avatar.jpg" class="mine-img">
                <input placeholder="Say somethings..." class="content-input" ref="comment">
                <button class="send-btn" @click="sendComment()">Send</button>
              </div>
              <div class="tab-view flex-view">
                <div class="count-text">Total of {{ commentData.length }} comments</div>
                <div class="tab-box flex-view" v-if="commentData.length > 0">
                  <span :class="sortIndex === 0? 'tab-select': ''" @click="sortCommentList('recent')">New</span>
                  <div class="line"></div>
                  <span :class="sortIndex === 1? 'tab-select': ''" @click="sortCommentList('hot')">Hot</span>
                </div>
              </div>
              <div class="comments-list">
                <div class="comment-item" v-for="item in commentData">
                  <div class="flex-item flex-view">
                    ...
                    <div class="float-right">
                      <span @click="like(item.id)">Recommand</span>
                      <span class="num">{{ item.like_count }}</span>
                    </div>
                  </div>
                  <p class="comment-content">{{ item.content }}</p>
...
import { addCart } from "@/api/index/cart";
  ...
  methods: {
    ...
    getThingDetail () {
      detailApi({id: this.thingId}).then(res => {
        ...
        if (this.$store.state.language === 'zh') {
          ...
          this.$message.error('Failed to obtain details')
        }
      })
    },
    addToWish () {
    ...
    share () {
      if (this.$store.state.language === 'zh') {
        ...
        let content = 'Share a very fun website ' + window.location.href
        let shareHref = 'http://service.weibo.com/share/share.php?title=' + content
        window.open(shareHref)
      }
    },
    handleCart (detailData) {
      addCart({userId: this.$store.state.user.userId, thingId: detailData.id, price: detailData.price}).then(res => {
        console.log(res)
      })
      if (this.$store.state.language === 'zh') {
