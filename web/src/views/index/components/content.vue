  ...
  <div  class="content">
    <div class="content-left">
      <div class="left-search-item">
        <h4>{{ lang.goodType[this.$store.state.language] }}</h4>
        <a-tree  v-if="this.$store.state.language === 'zh'" :tree-data="cData" :selected-keys="selectedKeys" @select="onSelect" style="min-height: 220px;">
        </a-tree>
        <a-tree  v-if="this.$store.state.language === 'en'" :tree-data="encData" :selected-keys="selectedKeys" @select="onSelect" style="min-height: 220px;">
        </a-tree>
      </div>
      <div class="left-search-item"><h4>{{ lang.tags[this.$store.state.language] }}</h4>
    ...
    <div class="content-right" v-if="this.$store.state.language == 'zh'">
      ...
      <a-spin :spinning="loading" style="min-height: 200px;">
        <div class="pc-thing-list flex-view">
          ...
          <div v-if="pageData.length <= 0 && !loading" class="no-data" style="">{{ lang.show[this.$store.state.language] }}</div>
        </div>
      </a-spin>
    ...
    <div class="content-right" v-if="this.$store.state.language == 'en'">
      <div class="top-select-view flex-view">
        <div class="order-view">
          <span class="title"></span>
          <span class="tab"
  ...
  methods: {
    ...
    initSider () {
      listClassificationList().then(res => {
        this.cData = JSON.parse(JSON.stringify(res.data));
        this.encData = JSON.parse(JSON.stringify(res.data));
        this.encData.forEach(item => {
        item.title = item.en_title;
        delete item.en_title; // 删除原来的title字段
        })
