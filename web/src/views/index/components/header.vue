<template>
  <div class="main-bar-view">
    ...
    <div class="search-entry">
      ...
      <input :placeholder="lang.searchKeyWord[this.language]" ref="keyword" @keyup.enter="search" />
    </div>
    <div class="right-view">
      <template>
        <label>EN&nbsp;<input type="radio" v-model="language" value="en" checked style="margin-top: 8px;"></label>
        <label>ZH&nbsp;<input type="radio" v-model="language" value="zh" style="margin-top: 8px;"></label>
      </template>
      <template v-if="$store.state.user.username">
        <a-dropdown>
          ...
          <a-menu slot="overlay">
            <a-menu-item>
              <a @click="goUserCenter('orderView')">{{ lang.orderCenter[this.language] }}</a>
            </a-menu-item>
            <a-menu-item>
              <a @click="goUserCenter('userInfoEditView')">{{ lang.settings[this.language] }}</a>
            </a-menu-item>
            <a-menu-item>
              <a @click="goUserCenter('CartView')">{{ lang.cart[this.language] }}</a>
            </a-menu-item>
            <a-menu-item>
              <a @click="quit()">{{ lang.logout[this.language] }}</a>
            </a-menu-item>
          </a-menu>
        </a-dropdown>
...
<!--        </div>-->
      ...
      <template v-else>
        <button class="login btn hidden-sm" @click="goLogin()">{{ lang.login[this.language] }}</button>
      </template>
      ...
      <div>
        <a-drawer
          :title="lang.message[this.language]"
          placement="right"
          :closable="true"
          :maskClosable="true"
          :visible="msgVisible"
...
export default {
  ...
  watch: {
    language(newLanguage) {
      this.$store.dispatch('setLan', newLanguage);
    }
  },
  created() {
    this.language = this.$store.state.language; // 设置radio按钮的选中状态
  },
  mounted () {
    this.getMessageList()
  },
