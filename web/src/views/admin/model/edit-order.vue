<template>
  <a-form-model
    ...
    <a-row v-if="this.$store.state.language === 'en'" :gutter="24">
      <a-col span="24">
        <a-form-model-item label="Goods" prop="thing">
          <a-select placeholder="Please select" allowClear v-model="form.thing">
            <template v-for="item in thingData">
              <a-select-option :key="item.id" :value="item.id">{{item.title}}</a-select-option>
            </template>
          </a-select>
      ...
      <a-col span="24">
        <a-form-model-item label="User" prop="user">
          <a-select placeholder="Please select" allowClear v-model="form.user">
            <template v-for="item in userData">
              <a-select-option :key="item.id" :value="item.id">{{item.username}}</a-select-option>
            </template>
          </a-select>
