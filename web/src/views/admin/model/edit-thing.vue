<template>
  <a-form-model
        ...
        <a-form-model-item label="分类" prop="classification">
          ...
          >
            <template v-for="item1 in cData">
              <a-tree-select-node :key="item1.id" :value="item1.id" :title="item1.title">
                <template v-for="item2 in item1.children">
                  <a-tree-select-node :key="item2.id" :value="item2.id" :title="item2.title"></a-tree-select-node>
                </template>
              </a-tree-select-node>
            </template>
          </a-tree-select>
    ...
    <a-row v-if="this.$store.state.language === 'en'" :gutter="24">
      <a-col span="24">
        <a-form-model-item label="Goods Name" prop="title">
          <a-input placeholder="Please enter" v-model="form.title"></a-input>
        </a-form-model-item>
      </a-col>
      <a-col span="12">
        <a-form-model-item label="classification" prop="classification">
          <a-tree-select
            v-model="form.classification"
            style="width: 100%"
            :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }"
      ...
      <a-col span="12">
        <a-form-model-item label="Tag">
          <a-select mode="multiple" placeholder="Please select" allowClear v-model="form.tag">
            <template v-for="item in tagData">
              <a-select-option :key="item.id" :value="item.id">{{item.title}}</a-select-option>
            </template>
          </a-select>
      ...
      <a-col span="24">
        <a-form-model-item label="Cover">
          ...
          >
            ...
            <p class="ant-upload-text">
              Please select the cover image to upload
            </p>
          </a-upload-dragger>
        </a-form-model-item>
      </a-col>
      ...
      <a-col span="24">
        <a-form-model-item label="content validity">
          <a-textarea placeholder="Please enter" v-model="form.description"></a-textarea>
        </a-form-model-item>
      </a-col>
      <a-col span="12">
        <a-form-model-item label="Price" prop="price">
          <a-input-number  placeholder="Please enter" :min="0" v-model="form.price" style="width: 100%;"></a-input-number>
        </a-form-model-item>
      </a-col>
      <a-col span="12">
        <a-form-model-item label="Status" prop="status">
          <a-select placeholder="Please select" allowClear v-model="form.status">
            <a-select-option key="0" value="0">Grounding</a-select-option>
            <a-select-option key="1" value="1">Off shelf</a-select-option>
          </a-select>
        </a-form-model-item>
      </a-col>
      <a-col span="12">
        <a-form-model-item label="Repertory" prop="repertory">
          <a-input-number placeholder="Please enter" :min="0" v-model="form.repertory" style="width: 100%;"></a-input-number>
        </a-form-model-item>
      </a-col>
    </a-row>
  </a-form-model>
