...
from myapp.models import Order, Thing, Cart
...
def create(request):
    ...
    if "type" in data:
        for i in json.loads(data['cart_data']):
            thing = Thing.objects.get(pk=i['thing'])
            count = i['number']
...
def delivery(request):
    ...
    if 'id' not in data or 'time' not in data:
        return APIResponse(code=1, msg='参数错误')
    if data['id'] is None or data['time'] is None:
        return APIResponse(code=1, msg='参数错误')

    order = Order.objects.get(pk=data['id'])
    order.delivery_time = data['time']
    order.save()
